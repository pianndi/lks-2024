class Game {
  ...
  constructor(canvas, context) {
    ...
    this.current = new Hexagon(this, this.width / 2, this.height - 110, 24, true, this.number)
  ...
  render() {
    ...
    this.ctx.textAlign = 'end'
    this.ctx.fillText(this.player2, this.width - this.width / 4 - 5, this.height - 70)
    this.ctx.textAlign = 'center'
    this.ctx.fillText(this.score[0], this.width / 4 + 10, this.height - 40)
    this.ctx.fillText(this.score[1], this.width - this.width / 4 + 10, this.height - 40)
  }
  generateHexagons() {
    const a = 2 * Math.PI / 6
    const r = 35
    for (let x = 60; x + r * Math.sin(a) < r * 10 * 2 - r; x += r * 2 * Math.sin(a)) {
      for (let y = 60, j = 0; y + r * (1 + Math.cos(a)) < r * 2 * 8 - r; y += r * (1 + Math.cos(a)), x += (-1) ** j++ * r * Math.sin(a)) {
        this.hexagons.push(new Hexagon(this, x, y, r, false))
      }
    }
    for (let i = 0; i < this.level; i++) {
      const filteredHex = this.hexagons.filter(item => !item.disabled)
      filteredHex[Math.floor(Math.random() * filteredHex.length)].disabled = true
    }
  ...
  saveScore() {
    const score = JSON.parse(localStorage.getItem('hex-leaderboard')) || new Array()
    score.push({
      player1: {
        name: this.player1,
        score: this.score[0]
...
function fetchLeaderboard() {
  const score = JSON.parse(localStorage.getItem('hex-leaderboard')) || new Array()
  if (sort.value == 'score') {
    score.sort((a, b) => (b.player1.score + b.player2.score) - (a.player1.score + a.player2.score))
  } else {
